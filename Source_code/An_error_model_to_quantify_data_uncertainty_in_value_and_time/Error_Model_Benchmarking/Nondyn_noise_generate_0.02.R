...
Nondyn_rep_0.02<-list()
for(i in 1:1000){
  Nondyn_rep_0.02[[i]]<-(100/Nondyn_WT_bio)*dynamic_exp_rep[[i]][which(rownames(dynamic_exp_true)%in%apply(Nonredundent_name,1,as.character)),]
  print(i)
}

save(Nondyn_rep_0.02,file="Nondyn_rep_0.02.RData")
###########################variance analysis
#########variance of input and parameter varied data
var_empirical_bio_para_0.02<-matrix(0,ncol=104,nrow = 93)
ind_j<-rep(c(0,15,30,60),26)/int_length+rep(seq(0,801*25,801),each=4)+1

for(i in 1:93){
  for(j in 1:104){
    var_empirical_bio_para_0.02[i,j]<-var(unlist(lapply(Nondyn_rep_0.02,function(x) x[i,ind_j[j]])))
    # print(c(i,j))
  }
}

save(var_empirical_bio_para_0.02,file="var_empirical_bio_para_0.02.RData")
########################################
#Generate tempral variance: biological + technical
########################################
# Nondyn_active_bio<-Nondyn_rep_0.02[[1]]
...
###########genrate temporal
Nondyn_active_temp_B5T5_parV_rep1000_N_0.02<-vector()
for(i in 1:rep_size){
  Nondyn_active_temp_B5T5_parV_rep1000_N_0.02<-rbind(Nondyn_active_temp_B5T5_parV_rep1000_N_0.02,temp_noise_generate(deltaN*5,deltaN*5,Nondyn_rep_0.02[[i]],1,i))
  print(i)
}

rownames(Nondyn_active_temp_B5T5_parV_rep1000_N_0.02)<-rep(apply(Nonredundent_name,1,as.character),rep_size)
save(Nondyn_active_temp_B5T5_parV_rep1000_N_0.02,file="Nondyn_active_temp_B5T5_parV_rep1000_N_0.02.RData")
#########variance of simulated data
var_ij<-matrix(0,ncol=104,nrow = 93)
for(i in 1:93){
  for(j in 1:104){
    var_ij[i,j]<-var(Nondyn_active_temp_B5T5_parV_rep1000_N_0.02[(c(i)+seq(0,(rep_size-1)*93,93)),j])
  }
}
var_empirical_pure_bio_V0.01_B5T5_rep1000_0.02<-var_ij
save(var_empirical_pure_bio_V0.01_B5T5_rep1000_0.02,file="var_empirical_pure_bio_V0.01_B5T5_rep1000_0.02.RData")
...
pdf("Temp_var_estimate_B5T5_parV_rep1000_0.02.pdf",width=5,height=5)
plot(as.matrix(t(var_empirical_pure_bio_V0.01_B5T5_rep1000_0.02),ncol=1),as.matrix(slope_list_GT[slope_ind,]^2*50*deltaN^2,ncol=1),xlim=c(0,2000),ylim=c(0,2000),xlab="Variance of simulated data",ylab="Estimated variance from theory",pch=16)
abline(coef = c(0,1),col="red",lwd=2)
graphics.off()
########################################
#Generate basal level:0.05
########################################Uniform[0,1]
Nondyn_active_temp_B5T5_parV_rep1000_N_0.02[Nondyn_active_temp_B5T5_parV_rep1000_N_0.02==0]<-0.05
########################################
...
##replicates: TB5,TT5
rep_num<-1000
Nondyn_bio_V0.01_B5T5_rep1000_0.02<-vector()
for(i in 1:rep_num){
  Nondyn_bio_V0.01_B5T5_rep1000_0.02<-cbind(Nondyn_bio_V0.01_B5T5_rep1000_0.02,noise_generate(Nondyn_active_temp_B5T5_parV_rep1000_N_0.02[((93*i-92):(93*i)),],0.01*deltaN^2,1,i)$dyn_data_Noise)
  print(i)
}
##########
save(Nondyn_bio_V0.01_B5T5_rep1000_0.02,file="Nondyn_bio_V0.01_B5T5_rep1000_0.02.RData")
################################################################
################################################################
################Empirical Variance################
#############TB5_TT5
var_empirical_bio_V0.01_B5T5_rep1000_0.02<-matrix(0,ncol=104,nrow = 93)
for(i in 1:93){
  for(j in 1:104){
    var_empirical_bio_V0.01_B5T5_rep1000_0.02[i,j]<-var(Nondyn_bio_V0.01_B5T5_rep1000_0.02[i,(j+seq(0,104*999,104))])
  }
}
save(var_empirical_bio_V0.01_B5T5_rep1000_0.02,file="var_empirical_bio_V0.01_B5T5_rep1000_0.02.RData")
########################################
################################Plot dot
########################################
...
#################################Plot##############
pdf("Input_noise_plot_0.02.pdf",width=10,height=14)
...
gene_raw<-rbind(Nondyn_bio_V0.01_B5T5_rep1000_0.02[,1:104],Nondyn_bio_V0.01_B5T5_rep1000_0.02[,105:208])
gene_plot<-gene_raw[rep(c(1:93),each=2)+rep(c(0,93),93),]

pdf("Gene_plot_0.02.pdf",width=20,height=14)
...
pdf("Gene_per_0.02.pdf",width=20,height=14)
